...
A Purchase Order is a demand that get generated by distributer to the Manfacturer based on the secondary order received from the Retailers , It will get created by distributor operator through distributor portal at distributor location.  
...
10. Fill in or Modify the remaining fields on the Purchase Order page as necessary.
...
12. **Provide the Purchase order quantity**
...
18. **Save Purchase Order ** 
On Saving of the Purchase Order document, 
    - Document of Purchase Order is recorded with generation of unique Purchase Order Transaction number for a Distributor. 
    - Line Item & quantity of details provided are added as part of Purchase order document as Open order
    - Off take scheme benefit applied are accounted and updated. 
    - Report data consolidation initiated for relevant data points. 
    - Purchase Order is ready to [Print](Purchase Order Print)

## Cancellation 
Cancellation of Purchase Order are action performed on Non-Invoiced Purchase Order after preparation of Purchase Order . The reason may vary like "Wrongly created Order ". Cancelled Order are considered as valid document and used in all reports
...
Cancellation of Purchase Order are restricted based on configuration & workflow, few points as below. 
  ...
  - Business level restriction may vary based on configuration.  
    - Invoiced Purchase Order are not allowed to cancel, instead before the delivery , Purchase invoice cancellation to be performed to maintain the logical accounting transactions. 
...
- Market Purchase Order 
  - A [Purchase Order](Purchase Order) records the customer's intent to buy the listed products at the stated price during the market visit by the salesperson


# Purchase Order Configuration 
This page list all direct configuration applicable for Purchase Order & related other module configuration which impacts Collections. 
